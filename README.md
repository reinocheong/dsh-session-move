...
- **Running sessions (fixed in v0.1.2)** — when the moved session is actively executing a task, the plugin now also **releases the agent** from the agents registry (emits `agent/disposed`), not just cancels its current turn. This is the load-bearing part: dsh's agent factory waits for BOTH the session and the agent to leave their registries (`waitForDrainingConfiguredIdentity` listens for `session/disposed` and `agent/disposed`) before rebuilding a live session; cancelling the turn alone leaves the agent registered, so the wait never resolves and the moved session can never be re-opened or resumed (symptom: you can type and hear the notification sound, but the agent never replies). The client also **seamlessly reloads the moved session's view** after a successful move — the host flags the resident conversation instance as `removed` ("session unavailable" input) and that flag cannot be reset in place, so the plugin switches to another listed session first (while the moved session is still unlisted, `sweepDeferred` genuinely drops its stale scope), refreshes the list, then switches back — the session re-opens cold from its new location with history intact in a few hundred milliseconds and no page reload. Any step that fails falls back to an automatic full page reload, so the view can never stay stuck on "session unavailable".
