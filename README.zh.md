...
- **运行中的会话（v0.1.2 修复）**——移动正在执行任务的会话时，除了取消当前回合，还会从 agents 注册表**释放 agent**（发出 `agent/disposed`）。这是关键：dsh 的 agent 工厂在重建会话前会等待「session 与 agent 双双离开注册表」（`waitForDrainingConfiguredIdentity` 监听 `session/disposed` 和 `agent/disposed`）；只取消回合而不释放 agent，等待会永远挂起，移动后的会话无法重新打开/恢复（表现为「能打字、只有提示音、没有回复」）。客户端在移动成功后也会**无感重载被移动的会话视图**——移动会把当前会话实例标记为 `removed`（输入框变成「会话不可用」），且该标志无法原位清除：插件会先切换到另一个会话（此时被移会话尚未重新入列，`sweepDeferred` 会真正销毁其陈旧 scope），刷新列表后再切回，从新位置冷加载重建（历史完整、输入框立即可用），整个过程只需约几百毫秒、无整页刷新；任何一步失败则自动**整页刷新**兜底，绝不会卡在「会话不可用」。
